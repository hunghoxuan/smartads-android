...
public abstract class AutoScrollHelper implements View.OnTouchListener {
	...
	private float[] mRelativeEdges = new float[] { RELATIVE_UNSPECIFIED,
	...
	private float[] mRelativeVelocity = new float[] { RELATIVE_UNSPECIFIED,
			RELATIVE_UNSPECIFIED };
	...
	private static final int DEFAULT_ACTIVATION_DELAY = ViewConfiguration
			.getTapTimeout();
	...
	public AutoScrollHelper(View target) {
		...
		final DisplayMetrics metrics = Resources.getSystem()
				.getDisplayMetrics();
		final int maxVelocity = (int) (DEFAULT_MAXIMUM_VELOCITY_DIPS
	 ...
	 * 
	 * @param horizontalMax
	 *            The maximum horizontal scrolling velocity, or {@link #NO_MAX}
	 *            to leave the relative value unconstrained.
	 * @param verticalMax
	 *            The maximum vertical scrolling velocity, or {@link #NO_MAX} to
	 *            leave the relative value unconstrained.
	 * @return The scroll helper, which may used to chain setter calls.
	 */
	public AutoScrollHelper setMaximumVelocity(float horizontalMax,
			float verticalMax) {
	 ...
	 * 
	 * @param horizontalMin
	 *            The minimum horizontal scrolling velocity, or {@link #NO_MIN}
	 *            to leave the relative value unconstrained.
	 * @param verticalMin
	 *            The minimum vertical scrolling velocity, or {@link #NO_MIN} to
	 *            leave the relative value unconstrained.
	 * @return The scroll helper, which may used to chain setter calls.
	 */
	public AutoScrollHelper setMinimumVelocity(float horizontalMin,
			float verticalMin) {
	 ...
	 * 
	 * @param horizontal
	 *            The target horizontal velocity as a fraction of the host view
	 *            width per second, or {@link #RELATIVE_UNSPECIFIED} to ignore.
	 * @param vertical
	 *            The target vertical velocity as a fraction of the host view
	 *            height per second, or {@link #RELATIVE_UNSPECIFIED} to ignore.
	 * @return The scroll helper, which may used to chain setter calls.
	 */
	 ...
	 * 
	 * @param horizontal
	 *            The horizontal edge size as a fraction of the host view width,
	 *            or {@link #RELATIVE_UNSPECIFIED} to always use the maximum
	 *            value.
	 * @param vertical
	 *            The vertical edge size as a fraction of the host view height,
	 *            or {@link #RELATIVE_UNSPECIFIED} to always use the maximum
	 *            value.
	 * @return The scroll helper, which may used to chain setter calls.
	 */
	 ...
	 * 
	 * @param horizontalMax
	 *            The maximum horizontal edge size in pixels, or {@link #NO_MAX}
	 *            to use the unconstrained calculated relative value.
	 * @param verticalMax
	 *            The maximum vertical edge size in pixels, or {@link #NO_MAX}
	 *            to use the unconstrained calculated relative value.
	 * @return The scroll helper, which may used to chain setter calls.
	 */
	public AutoScrollHelper setMaximumEdges(float horizontalMax,
			float verticalMax) {
	 ...
	 * <p>
	 * If {@link #isExclusive()} is false, always returns false so that the host
	 * view may handle touch events. Otherwise, returns true when automatic
	 * scrolling is active and false otherwise.
	 */
	@Override
	public boolean onTouch(View v, MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			final float xTargetVelocity = computeTargetVelocity(HORIZONTAL,
					event.getX(), v.getWidth(), mTarget.getWidth());
			final float yTargetVelocity = computeTargetVelocity(VERTICAL,
					event.getY(), v.getHeight(), mTarget.getHeight());
	...
	private boolean shouldAnimate() {
		...
		return verticalDirection != 0
				&& canTargetScrollVertically(verticalDirection)
				|| horizontalDirection != 0
				&& canTargetScrollHorizontally(horizontalDirection);
	}
	...
	private void startAnimating() {
		...
		if (!mAlreadyDelayed && mActivationDelay > 0) {
			ViewCompat.postOnAnimationDelayed(mTarget, mRunnable,
					mActivationDelay);
		} else {
			mRunnable.run();
		}
	...
	private float computeTargetVelocity(int direction, float coordinate,
			float srcSize, float dstSize) {
		final float relativeEdge = mRelativeEdges[direction];
		final float maximumEdge = mMaximumEdges[direction];
		final float value = getEdgeValue(relativeEdge, srcSize, maximumEdge,
				coordinate);
		if (value == 0) {
		...
		if (value > 0) {
			return constrain(value * targetVelocity, minimumVelocity,
					maximumVelocity);
		} else {
			return -constrain(-value * targetVelocity, minimumVelocity,
					maximumVelocity);
	 ...
	 * 
	 * @param relativeValue
	 *            The size of the inset relative to the total size.
	 * @param size
	 *            Total size.
	 * @param maxValue
	 *            The maximum size of the inset, used to clamp (relative *
	 *            total).
	 * @param current
	 *            Touch position within within the total size.
	 * @return Interpolated value of the touch position within the edge.
	 */
	private float getEdgeValue(float relativeValue, float size, float maxValue,
			float current) {
	...
	private void cancelTargetTouch() {
		...
		final MotionEvent cancel = MotionEvent.obtain(eventTime, eventTime,
	...
	private static class ClampedScroller {
		...
		public void requestStop() {
			final long currentTime = AnimationUtils
					.currentAnimationTimeMillis();
			mEffectiveRampDown = constrain((int) (currentTime - mStartTime), 0,
					mRampDownDuration);
		...
		public boolean isFinished() {
			return mStopTime > 0
					&& AnimationUtils.currentAnimationTimeMillis() > mStopTime
							+ mEffectiveRampDown;
		}
		...
		private float getValueAt(long currentTime) {
			if (currentTime < mStartTime) {
				...
				return 0.5f * constrain(elapsedSinceStart
						/ (float) mRampUpDuration, 0, 1);
			} else {
				final long elapsedSinceEnd = currentTime - mStopTime;
				return (1 - mStopValue)
						+ mStopValue
						* constrain(elapsedSinceEnd
								/ (float) mEffectiveRampDown, 0, 1);
			}
		}

		/**
		 * Interpolates the value along a parabolic curve corresponding to the
		 * equation <code>y = -4x * (x-1)</code>.
		 * 
		 * @param value
		 *            The value to interpolate, between 0 and 1.
