...
public class TwoWayView extends AdapterView<ListAdapter> implements
	 ...
	 * valid if the choice mode has been set to {@link ChoiceMode#SINGLE} or
	 ...
	 * choice mode has been set to {@link ChoiceMode#SINGLE}.
	 * 
	 ...
	 * any choice behavior ({@link ChoiceMode#NONE}). By setting the choiceMode
	 * to {@link ChoiceMode#SINGLE}, the List allows up to one item to be in a
	 * chosen state. By setting the choiceMode to {@link ChoiceMode#MULTIPLE},
	 * the list allows any number of items to be chosen.
	 * 
	 * @param choiceMode
	 *            One of {@link ChoiceMode#NONE}, {@link ChoiceMode#SINGLE}, or
	 *            {@link ChoiceMode#MULTIPLE}
	 */
	 ...
	 * from the top/left edge of the view, according to the orientation. If this
	 * is impossible, (e.g. the offset would scroll the first or last item
	 * beyond the boundaries of the list) it will get as close as possible. The
	 ...
	 *            top/left of the view when scrolling is finished
	 * @param duration
	...
	public void smoothScrollBy(int distance, int duration) {
		...
		if (distance == 0
			...
			mScroller.startScroll(mIsVertical ? 0 : initialPosition,
					mIsVertical ? initialPosition : 0, mIsVertical ? 0
							: -distance, mIsVertical ? -distance : 0, duration);
	...
	private View makeAndAddView(int position, int offset, boolean flow,
		...
		// Compensate item margin on the first item of the list if the item
		// margin
		// is negative to avoid incorrect offset for the very first child.
		if (mIsVertical) {
			top = offset
					- (mItemMargin < 0 && position == 0 && !flow ? mItemMargin
							: 0);
			left = getPaddingLeft();
		} else {
			top = getPaddingTop();
